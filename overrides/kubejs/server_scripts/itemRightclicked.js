...
onEvent('block.right_click', event => {
    ...
    if (! event.block.entityData) event.cancel(); return;
    if (! event.block.entityData.id || event.block.entityData.id !== 'create:blaze_heater') event.cancel(); return;

    // Prevent Straw duping because it's annoying!
    if (event.item.id == 'createaddition:straw' && event.level.dimension != 'cae:void') {
